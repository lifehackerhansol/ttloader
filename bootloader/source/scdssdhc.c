...
static void SCDS_SDSendR0Command(u8 sdio, u32 parameter)
{
	SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(sdio, parameter, SCDS_SD_HOST_NORESPONSE), initialized ? 0 : 0x1000);
	while(SCDS_IsSDHostBusy());
}

static u32 SCDS_SDSendR1Command(u8 sdio, u32 parameter)
{
	SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(sdio, parameter, SCDS_SD_HOST_READ_4B), initialized ? 0 : 0x1000);
	while(SCDS_IsSDHostBusy());
...
static void SCDS_SDSendR2Command(u8 sdio, u32 parameter)
{
	SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(sdio, parameter, SCDS_SD_HOST_READ_4B_MULTI), initialized ? 0 : 0x1000);
	while(SCDS_IsSDHostBusy());
	...
	for(int i=0; i < 4; i++)
	{
		SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(sdio, parameter, SCDS_SD_HOST_NEXT_4B), initialized ? 0 : 0x1000);
		while(SCDS_IsSDHostBusy());
		// TODO: parse this response
		SCDS_SendCommand(SCDS_CMD_SD_HOST_RESPONSE, 0);
	}
...
{
	u32 ret;
	SCDS_ReadCardData(command, SCDS_CTRL_READ_4B | MCCNT1_LATENCY1(latency), &ret, 1);
	return ret;
}
...
bool SCDS_SDInitialize(void)
{
	...
	SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(0, 0, SCDS_SD_HOST_SEND_CLK), 0x1000);

	// CMD0
	SCDS_SDSendR0Command(0, 0);

	// CMD8
	SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(8, 0x1AA, SCDS_SD_HOST_READ_4B), 0x1000);
	...
	{
		// CMD55
		SCDS_SendCommand(SCDS_CMD_SD_HOST_PARAM(55, 0, SCDS_SD_HOST_READ_4B), 0x1000);
		retryCount = 9999;
		while(SCDS_IsSDHostBusy())
		{
			if (--retryCount == 0)
		...
		if(isSD20)
			parameter |= BIT(30);
		response = SCDS_SDSendR1Command(41, parameter);
	} while(!(response & BIT(31)));

	isSDHC = response & BIT(30) ? 1 : 0;

	// CMD2
	SCDS_SDSendR2Command(2, 0);

	// CMD3
	response = SCDS_SDSendR1Command(3, 0);
	u32 sdio_rca = response & 0xFFFF0000;

	// CMD7
	SCDS_SDSendR1Command(7, sdio_rca);

	// ACMD6
	SCDS_SDSendR1Command(55, sdio_rca);
	SCDS_SDSendR1Command(6, 2);
...
void SCDS_SDWriteSingleSector(u32 sector, const void *buffer)
{
	...
	SCDS_SDSendR1Command(24, isSDHC ? sector : sector << 9);
...
{
	// instruct cart where to write
	SCDS_SDSendR1Command(25, isSDHC ? sector : sector << 9);
